Attribute VB_Name = "ROZDZIELACZ_CLEANER"
Sub SplitByIDPHAcrossSheetsWithFolderSelection()
    ...
    ' Iteracja po wszystkich zak�adkach w pliku, z pomini�ciem arkusza MENU
    ...
    For Each ID In uniqueIDs
        ...
        ' Iteracja po arkuszach w pliku g��wnym, z pomini�ciem arkusza MENU
        For Each ws In ThisWorkbook.Sheets
            If ws.Name <> "MENU" Then ' Sprawdzamy, czy arkusz nie jest MENU
                ' Sprawdzenie, czy w arkuszu znajduj� si� dane dla danego ID_PH
                dataFound = False
                ...
                For i = 2 To ws.Cells(ws.Rows.Count, "A").End(xlUp).Row
                    If ws.Cells(i, 1).Value = ID Then
                        If newSheet Is Nothing Then
                            ' Tworzenie nowego arkusza, je�li jeszcze nie zosta� stworzony
                            Set newSheet = newWorkbook.Sheets.Add
                            newSheet.Name = ws.Name ' Zachowanie nazwy arkusza
                            ws.Rows(1).Copy Destination:=newSheet.Rows(1) ' Kopiowanie nag��wk�w
                        End If
                        ' Kopiowanie danych
                ...
                ' Je�li nie znaleziono danych, nie dodawaj arkusza
